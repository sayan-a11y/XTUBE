...
export function MidRollAdsPage() {
  ...
  const { ads: allAds, stats, deleteAd, toggleAdStatus, fetchAds } = useRealtimeAds({ position: 'mid-roll' })
  ...
  const processSelectedFile = useCallback((file: File) => {
    ...
    if (isVideo) {
      ...
      tempVid.onloadedmetadata = () => {
        const roundedDuration = Math.round(tempVid.duration || 15)
    ...
    try {
      ...
      const res = await fetch('/api/ads', {
        ...
        body: JSON.stringify({
          ...
          adDuration: fileDetails?.duration || (adTab === 'video' ? 15 : 0),
          skipAfter: 5,
  ...
  const mappedAds: MidRollAd[] = useMemo(() => allAds.map((ad, i) => ({
    id: ad.id,
    name: ad.title,
    type: (ad.type === 'video' || ad.mediaFormat === 'mp4' || ad.mediaFormat === 'webm' ? 'Video' : 'Image') as MidRollAd['type'],
    ...
    status: (ad.isActive ? 'Active' : 'Paused') as MidRollAd['status'],
  ...
  const donutData = useMemo(() => [
    { name: 'Video Ads', value: allAds.filter(a => ['mp4', 'webm', 'mov', 'video'].includes(a.type) || ['mp4', 'webm', 'mov'].includes(a.mediaFormat)).reduce((s, a) => s + a.impressions, 0) || 2800 },
    { name: 'Image Ads', value: allAds.filter(a => !['mp4', 'webm', 'mov', 'video'].includes(a.type) && !['mp4', 'webm', 'mov'].includes(a.mediaFormat)).reduce((s, a) => s + a.impressions, 0) || 750 },
  ], [allAds])
      ...
      <div className="min-h-full p-3 lg:p-5 space-y-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-white/40">Create and manage mid-roll stream video &amp; image ads</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          <StatCard title="Total Mid-Roll Ads" value={String(stats.totalAds)} change="+14.1%" icon={Megaphone} color={STAT_COLORS[0]} delay={0} index={0} />
          <StatCard title="Active Ads" value={String(stats.activeAds)} change="+11.3%" icon={Radio} color={STAT_COLORS[1]} delay={0.05} index={1} />
          <StatCard title="Impressions" value={formatAdNumber(stats.totalImpressions)} change="+19.2%" icon={Eye} color={STAT_COLORS[2]} delay={0.1} index={2} />
          <StatCard title="CTR" value={stats.avgCTR.toFixed(2) + '%'} change="+9.1%" icon={MousePointer} color={STAT_COLORS[3]} delay={0.15} index={3} />
          <StatCard title="Revenue" value={formatAdRevenue(stats.totalRevenue)} change="+15.2%" icon={DollarSign} color={STAT_COLORS[4]} delay={0.2} index={4} />
        </div>
            ...
            <div className="p-3 lg:p-4">
              ...
              <div className="mb-4 flex items-center gap-0 border-b border-white/5">
                ...
                >
                  ...
                  {adTab === 'video' && (
                    <motion.div
                      layoutId="midroll-tab-indicator"
            ...
            <div className="overflow-hidden rounded-xl border border-white/5 bg-[#111111]/80 backdrop-blur-xl">
              <div className="p-3 lg:p-4">
                ...
                <div className="relative aspect-video overflow-hidden rounded-lg bg-black group border border-white/5 shadow-2xl">
                  ...
                  {(!mediaUrl || adTab === 'image') && (
                    <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-2 text-center p-4">
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-[#ff2e2e]">Sponsored Mid-Roll</p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent px-3 pb-2 pt-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] text-white/50">00:04 / {fileDetails?.duration ? `00:${String(fileDetails.duration).padStart(2, '0')}` : '00:15'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  {[
                    { label: 'Deployment Format', value: adTab === 'video' ? 'Video Mid-Roll Clip' : 'Image Mid-Roll Banner' },
                ...
                <ResponsiveContainer width="99%" height="100%">
                  <PieChart>
                    ...
                    <text x="50%" y="44%" textAnchor="middle" dominantBaseline="middle" className="fill-white text-xs font-bold">
                      {stats.totalImpressions > 0 ? (stats.totalImpressions / 1000).toFixed(0) + 'K' : '28.1K'}
                    </text>
        ...
        >
          <div className="p-3 lg:p-4">
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-base font-bold text-white">Mid-Roll Campaigns list</h2>
              <div className="flex items-center gap-2">
